...
def run(data,
    ...
    if not training:
        ...
        dataloader = create_dataloader_rgb_ir(val_path_rgb, val_path_ir,imgsz, batch_size // WORLD_SIZE * 2, gs, model.names, single_cls,
                                       hyp=None, cache=None, rect=True, rank=-1,
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        # Inference
        # out, train_out = model(img_rgb,img_ir) if training else model(img_rgb,img_ir, augment=augment, val=True)  # inference, loss outputs
        out = model(img_rgb, img_ir)  # inference and training outputs
...
def parse_opt():
    ...
    # parser.add_argument('--weights', nargs='+', type=str, default=ROOT / './weights-darknet/best.pt', help='model.pt path(s)')
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / './weights-resnet/best.pt', help='model.pt path(s)')
