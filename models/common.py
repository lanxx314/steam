...
class SelfAttention(nn.Module):
    ...
    def __init__(self, d_model, d_k, d_v, h,sr_ratio, attn_pdrop=.1, resid_pdrop=.1):
        ...
        self.sig_factor = nn.Sigmoid()
        self.sparse_factor = nn.Parameter(torch.tensor([0.0], requires_grad = True))
...
# yuan attention 
        ...
        # the core code block
        # topk=100
        print("factor target: ", self.sparse_factor)
        if (ksraN>101 and ksraN<10000):
            topk=int(ksraN*self.sig_factor(self.sparse_factor))       # pao preAtten
            # 0.8 
...
class CFA(nn.Module):
    ...
    """
    # 在模块初始化中传入参数
    def __init__(self, c1, c2, d_model, vert_anchors, horz_anchors, k=1, s=1, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        super(CFA, self).__init__()
        self.n_embd = d_model
        self.vert_anchors = vert_anchors  # 用来控制，Transformer的复杂度，特征图的大小均降低为原来额1/2
        self.horz_anchors = horz_anchors
        self.conv = nn.Conv2d(c1, c2, k, s, autopad(k, p), groups=g, bias=False)
        self.bn = nn.BatchNorm2d(c2)
        self.act = nn.Hardswish() if act else nn.Identity()
        self.ca_rgb = ChannelAttention1(c2)
        self.ca_ir = ChannelAttention1(c2)
        self.sa = SpatialAttention()
        self.dwconv1 = nn.Conv2d(c2* 2, c2 * 2, kernel_size=7, padding=3, groups=c2)
        self.dc1 = ChannelAttention(c2*2)
        self.dwconv2 = nn.Conv2d(c2* 2, c2 * 2, kernel_size=7, padding=3, groups=c2)
        self.dc2 = ChannelAttention(c2 * 2)
        self.cbam = Conv_CBAM(c2 * 2, c2 * 2)

        self.map_rgb = DWConv(c1 * 2, c1)
        self.map_ir = DWConv(c1 * 2, c1)
    ...
    def forward(self, x):
        ...
        #return rgb_fea_out_map, ir_fea_out_map
	
    #"""
        # 各自做ca
        path_all = torch.cat([rgb_fea_out_map, ir_fea_out_map], dim=1)
        ca_rgb_para = self.ca_rgb(rgb_fea_out_map)
        ca_ir_para = self.ca_ir(ir_fea_out_map)

        rgb_ir_pro = ca_rgb_para*ca_ir_para

        rgb_path1 = rgb_fea_out_map*(rgb_ir_pro + ca_rgb_para)
        ir_path1 = ir_fea_out_map*(rgb_ir_pro + ca_ir_para)

        rgb_path2 = rgb_fea_out_map * (1-rgb_ir_pro)
        ir_path2 = ir_fea_out_map * (1-rgb_ir_pro)

        path1 = torch.cat([rgb_path1, ir_path1], dim=1) + path_all
        ca_path1 = self.dwconv1(path1)
        ca_path1 = self.dc1(ca_path1)

        path2 = torch.cat([rgb_path2, ir_path2], dim=1) + path_all
        ca_path2 = self.dwconv2(path2)
        ca_path2 = self.dc2(ca_path2)

        add_fea = ca_path1+ca_path2

        add_rgb = self.map_rgb(add_fea)
        add_ir = self.map_ir(add_fea)

        x_rgb = torch.add(rgb_fea_out_map, add_rgb)  # concat

        x_ir = torch.add(ir_fea_out_map, add_ir)  # concat
...
class SpaAtten(nn.Module):
    def __init__(
        ...
        super(SpaAtten, self).__init__()  # 调用 nn.Module 的构造函数
        self.dim = dim  # 特征维度
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # 计算最终的注意力加权
        x = x * x_h_attn * x_w_attn # 广播机制


        return  x * x_h_attn * x_w_attn # 广播机制
...
class GPT(nn.Module):
    ...
    """
    [[19,21], 1, GPT, [1024,1,10,10,6,20]],    # 22-P5/32     [channel , sra  ,h , w] 20*20  作为特征图传入检测头
    [[19,22], 1, Add2, [1024,0]],    # 23-P5/32 stream one x+trans[0] 可见光分支
    [[21,22], 1, Add2, [1024,1]],    # 24-P5/32 stream two x+trans[1] 红外图像分支，用于后续计算

    第一层： [[3,7], 1, GPT, [256,4,40,40,6,80]],   # 8-P3/8    [channel , sra  ,h , w, number]  80*80
    """
    ...
    def __init__(self, d_model, sr_ratio, vert_anchors, horz_anchors, n_layer, mapreal,
        ...
        self.n_embd = d_model  # 通道数，拉伸后向量的个数
        self.vert_anchors = vert_anchors  # 用来控制，Transformer的复杂度，特征图的大小均降低为原来1/2
        self.horz_anchors = horz_anchors
        ...
        # 空间特征预处理
        self.spa_atten_rgb = SpaAtten(dim=d_model)
        self.spa_atten_ir = SpaAtten(dim=d_model)

        # positional embedding parameter (learnable), rgb_fea + ir_fea
        self.pos_emb = nn.Parameter(
            torch.zeros(1, 2 * vert_anchors * horz_anchors, self.n_embd))  # 可训练的位置编码   #  拼接后，维度变为2倍

        # transformer  dim:(B,2*H*W, C)
        self.trans_blocks = nn.Sequential(
            *[myTransformerBlock(d_model, d_k, d_v, h, block_exp, attn_pdrop, resid_pdrop, sr_ratio)
        ...
        self.CBAM_rgb = Conv_CBAM(d_model, d_model)
        self.CBAM_ir = Conv_CBAM(d_model, d_model)

        self.mapconv_rgb = nn.Conv2d(d_model * 2, d_model, kernel_size=1, stride=1, padding=0)
        self.mapconv_ir = nn.Conv2d(d_model * 2, d_model, kernel_size=1, stride=1, padding=0)

        self.cfa = CFA(d_model, d_model, d_model, vert_anchors, horz_anchors)
        self.eucb1 = EUCB(d_model, d_model)
        self.eucb2 = EUCB(d_model, d_model)
    ...
    def forward(self, x):
        ...
        """
        rgb_fea = x[0]  # rgb_fea (tensor): dim:(B, C, H, W)   # 数据以5个维度在网络中流动
        ir_fea = x[1]  # ir_fea (tensor): dim:(B, C, H, W)
        assert rgb_fea.shape[0] == ir_fea.shape[0]  # 判断特征图的大小是否一样
        ...
        """
        # SpaAtten
        rgb_fea = self.spa_atten_rgb(rgb_fea)
        ir_fea = self.spa_atten_ir(ir_fea)

        # -------------------------------------------------------------------------
        # AvgPooling
        # -------------------------------------------------------------------------
        # AvgPooling for reduce the dimension due to expensive computation
        rgb_fea = self.avgpool(rgb_fea)  # self.avgpool = nn.AdaptiveAvgPool2d((self.vert_anchors, self.horz_anchors))，水平和垂直方向输出特征图大小
        ir_fea = self.avgpool(ir_fea)  # 降低维度   dim:(bs, c, H, W)

        # -------------------------------------------------------------------------
        # Transformer
        # -------------------------------------------------------------------------
        # pad token embeddings along number of tokens dimension
        rgb_fea_flat = rgb_fea.view(bs, c, -1)  # flatten the feature   dim:(bs, c, H*W)
        ir_fea_flat = ir_fea.view(bs, c, -1)  # flatten the feature     dim:(bs, c, H*W)
        token_embeddings = torch.cat([rgb_fea_flat, ir_fea_flat], dim=2)  # concat，H*W维度拼接  dim:(bs, c, 2*H*W)
        token_embeddings = token_embeddings.permute(0, 2, 1).contiguous()  # dim:(B,2*H*W, C)

        # 加入位置编码:self.pos_emb = nn.Parameter(torch.zeros(1, 2 * vert_anchors * horz_anchors, self.n_embd)) # 可训练的位置编码   #  拼接后，维度变为2倍
        # transformer
        x = self.drop(
            self.pos_emb + token_embeddings)  # sum positional embedding and token    dim:(B, 2*H*W, C)  随机失活一部分，dropout

        # 传入transformer 单元 dim:(B,2*H*W, C)
        ...
        EAM 模块部分
        """
        rgb_fea_out, ir_fea_out = self.cfa(x)

        rgb_fea_out = F.interpolate(rgb_fea_out, size=([h, w]), mode='bilinear')
        ir_fea_out = F.interpolate(ir_fea_out, size=([h, w]), mode='bilinear')

        rgb_fea_out = self.eucb1(rgb_fea_out)
        ir_fea_out = self.eucb2(ir_fea_out)
...
class ChannelAttention1(nn.Module):
    ...
    def forward(self, x):
        avg_out = self.f2(self.relu(self.f1(self.avg_pool(x))))
        max_out = self.f2(self.relu(self.f1(self.max_pool(x))))
        #out = self.tanh(avg_out + max_out)  # NOT GOOD CHANGETO 1-2*SIGMOID
        out = 2*self.sigmoid(avg_out + max_out)-1
        return out
